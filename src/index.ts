...
class EmberPlusInstance extends InstanceBase<EmberPlusConfig> {
	...
	private updateCompanionBits(): void {
		if (this.config.monitoredParameters && this.config.monitoredParameters.length > 1)
			this.setActionDefinitions(GetActionsList(this, this.client, this.config))
		else this.setActionDefinitions(GetActionsListNonSelection(this, this.client))
	...
	private setupMonitoredParams(): void {
		this.config.monitoredParameters = [
			{
				node: new EmberModel.QualifiedElementImpl('0', new EmberModel.ParameterImpl(EmberModel.ParameterType.Null)),
				label: 'none_selected_use_Path_input',
			},
		]
	...
	private async getMonitoredParamValues() {
		for (const param of this.config.monitoredParameters ?? []) {
			try {
				await this.handleChangedValue(param.label, param.node)
			} catch (e) {
				this.log('error', 'Failed to update node "' + param.label + '": ' + e)
			}
		}
	}
	...
	private async handleChangedValue(path: string, node: TreeElement<EmberElement> | undefined) {
		if (node?.contents?.type == ElementType.Parameter) {
			...
			if (node.contents.parameterType == EmberModel.ParameterType.Boolean) value = node.contents.value as boolean
			else if (node.contents.parameterType == EmberModel.ParameterType.Integer) {
				if (node.contents.factor !== undefined) value = (node.contents.value! as number) / node.contents.factor
				else value = node.contents.value as number
			...
			else value = node.contents.value as string
